...
do
    ...
    function LinearSpring.canSleep<T>(self)
        if magnitudeSq(self.v) > SLEEP_VELOCITY_SQ_LIMIT then return false end
...
do
    ...
    local function rotateAxis(r0: Vector3, c1: CFrame)
        ...
        if mag > 1e-6 then c0 = fromAxisAngle(r0.Unit, mag) end
        return c0 * c1
    end
...
do
    ...
    function CFrameSpring.new(
        ...
        return setmetatable({
            ...
            _rotation = RotationSpring.new(
                dampingRatio,
                frequency,
                valueCurrent.Rotation,
                valueGoal.Rotation
            ),
        }, CFrameSpring)
    end
...
do
    local function inverseGammaCorrectD65(c)
        return c < 0.0404482362771076 and c / 12.92
            or 0.87941546140213 * (c + 0.055) ^ 2.4
    end

    local function gammaCorrectD65(c)
        return c < 3.1306684425e-3 and 12.92 * c
    ...
    function rgbToLuv(value: Color3): { number }
        ...
        -- sRGB -> xyz
        local x = 0.9257063972951867 * r
            - 0.8333736323779866 * g
            - 0.09209820666085898 * b
        local y = 0.2125862307855956 * r
            + 0.71517030370341085 * g
            + 0.0722004986433362 * b
        local z = 3.6590806972265883 * r
            + 11.4426895800574232 * g
            + 4.1149915024264843 * b

        -- xyz -> scaled cieluv
        local l = y > 0.008856451679035631 and 116 * y ^ (1 / 3) - 16
            or 903.296296296296 * y
    ...
    function luvToRgb(value: { number }): Color3
        ...
        -- cieluv -> xyz
        local y = (l + 16) / 116
        y = y > 0.206896551724137931 and y * y * y
            or 0.12841854934601665 * y - 0.01771290335807126
        local x = y * u / v
...
local PSEUDO_PROPERTIES: PropertyOverride = {
    Pivot = {
        class = "PVInstance",
        get = function(inst: PVInstance) return inst:GetPivot() end,
        set = function(inst: PVInstance, value: CFrame) inst:PivotTo(value) end,
    },
    Scale = {
        class = "Model",
        get = function(inst: Model) return inst:GetScale() end,
...
local function processSprings(
    springStates: typeof(springStates_other),
    dt: number
)
...
RunService.PreSimulation:Connect(
    function(dt) processSprings(springStates_other, dt) end
)

RunService.PostSimulation:Connect(
    function(dt) processSprings(springStates_render, dt) end
)
...
local function assertType(
    ...
    if not expectedType:find(typeof(value)) then
        error(
            `bad argument #{argNum} to {fnName} ({expectedType} expected, got {typeof(
                value
            )})`,
            3
        )
...
function spr.target(
    ...
    if frequency ~= frequency or frequency < 0 then
        error(
            ("expected undamped frequency >= 0; got %.2f"):format(frequency),
            2
        )
    end
    ...
    local targetRecord = (
        if instance:IsA("Camera")
            then springStates_render
            else springStates_other
    ) :: { [Instance]: { [string]: any } }
    ...
    for propName, propTarget in properties do
        ...
        if STRICT_RUNTIME_TYPES and typeof(propTarget) ~= typeof(propValue) then
            error(
                `bad property {propName} to spr.target ({typeof(propValue)} expected, got {typeof(
                    propTarget
                )})`,
                2
            )
    ...
    if not next(state) then targetRecord[instance] = nil end
end
...
function spr.stop(instance: Instance, property: string?)
    ...
    if property then
        local state = springStates_other[instance]
            or springStates_render[instance]
        if state then state[property] = nil end
    else
